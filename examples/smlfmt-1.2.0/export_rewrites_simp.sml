...
fun computeUpdates file body = let
  ...
  fun expandBox (start, stop) = (expandLeft start, expandRight (stop - 1) + 1)
  ...
  fun isSameLine s1 s2 = #1 (getLineCol s1) = #1 (getLineCol s2)
  ...
  fun sentinelBox box = (box, sentinel)
  ...
  fun destQuote q =
      case q of
        ...
        | _ => raise Bind
  fun destString exp = let
    val (StringConstant id) = exp
  in id end
  fun stringQuotes (i, s) = (i, i + String.size s - 1)
  fun stripStringQuotes s = String.substring (s, 1, String.size s - 2)
  fun destSingList exp = let
    val List {elems, right=SOME right, ...} = exp
    val {args = [arg], ...} = elems
  in (arg, right) end
  fun destExportRewrites dec = let
    val (val_, eq, fname, arg) = destCall dec
    val _ = if not $ (#2 fname) = "export_rewrites" then raise Bind else ()
    val (elem, right) = destSingList arg
  in (val_, stripStringQuotes (#2 (destString elem)), right) end
  fun destExportRewritesTotal dec =
      SOME (destExportRewrites dec) handle Bind => NONE
  fun addAttrAux (attr: string) (id: ident) (attrs: kvals attrs) =
    case attrs of
        NONE => [(identBox id, String.concat [#2 id, "[", attr, "]"])]
     |  SOME {right=SOME right, attrs = {args, ...}, ...} =>
        if List.exists (fn {key, ...} => #2 key = attr) args
        then []
        else [((right, right+1), String.concat [",", attr, "]"])]
     | _ => raise Fail "bad attribute parsing"
  fun processExportRewrites (val_, name, right) dec =
      case dec of
          (HOLTheoremDecl {id, attrs, ...}) =>
          if #2 id = name then (sentinelBox (val_, right+1))::(addAttrAux "simp" id attrs) else []
        | (HOLSimpleThm {id, attrs, ...}) =>
          if #2 id = name then (sentinelBox (val_, right+1))::(addAttrAux "simp" id attrs) else []
        | (HOLDefinition {id, attrs, ...}) =>
          if #2 id = name then (sentinelBox (val_, right+1))::(addAttrAux "simp" id attrs) else []
        | _  => []
  fun computeUpdates (dec1::(rest as dec2::_)) =
      (case destExportRewritesTotal dec1 of
           NONE => computeUpdates rest
         | SOME exportRewrite =>
           (processExportRewrites exportRewrite dec2) @ computeUpdates rest)
    | computeUpdates _ = []
in computeUpdates (List.rev decs) end
