...
fun computeUpdates file body = let
  ...
  fun computeUpdatesDec dec = let
    ...
    val _ = if not $ (#2 fname) = "prove" then raise Bind else ()
    val (left, quote, comma0, exp, right) = destTuple arg
    val (termStart, openq, closeq) = destTermApp quote
    (* Remove opening quote of HOL term *)
    val openq_box = identBox openq
    (* yoink term *)
    val term_upd = removeBox (termStart, termStart+4)
    ...
    (* val ... = ... ("
       ==>
       Theorem  *)
    val thm_box = (val_, val_ + 3)
    val thm_upd = updToCol0 thm_box "Theorem"
    (*  *)
    val eq_upd = (expandBox (eq, eq+1),
                  "[local]:"
                  ^ (if isSameLine eq (#2 openq_box) then "\n" else ""))
    (* *)
    val fun_upd = removeBox $ expandBox $ identBox fname
    val left_upd = ((left, left + 1), " ")
    (* `.., ==> Proof *)
    val proof_box = expandBoxLeft (#1 closeq, comma0 + 1)
    val proof_upd = updToCol0 proof_box "Proof"
    (* ) ==> QED *)
    val qed_box = expandBoxLeft (right, right + 1)
    val qed_upd = updToCol0 qed_box "QED"
  in [thm_upd, term_upd, fun_upd, left_upd, eq_upd, openq_upd, proof_upd, qed_upd] end handle Bind => []
