...
fun computeUpdates file body = let
  ...
  fun computeUpdatesDec dec = let
    ...
    (*  *)
    val _ = if not $ (#2 fname) = "Hol_datatype" then raise Bind else ()
    val (openq, closeq) = destQuote arg
    (*  *)
    val thm_box = expandBox (val_, #2 (identBox fname))
    val thm_upd = updToCol0 thm_box "Datatype:"
    (*  *)
    ...
    (*  *)
    val closeq_box = expandBox $ identBox closeq
    val closeq_upd = updToCol0 closeq_box "End"
  in [thm_upd, openq_upd, closeq_upd] end handle Bind => []
