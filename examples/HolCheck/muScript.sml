...
RW_TAC std_ss [STATES_def,RVNEG_def,wfKS_def], (* /\ *)
RW_TAC std_ss [STATES_def,RVNEG_def,wfKS_def], (* \/ *)
MAP_EVERY Q.X_GEN_TAC [`s`, `e`, `Q`, `st`] THEN
...
val MU_BISIM_STATES = store_thm(
  ...
  Induct_on `f` THENL [
    ...
    FULL_SIMP_TAC std_ss [MU_SAT_NEG,SUB_AP_NEG] THEN RES_TAC,
    ...
    (* AP *) REPEAT STRIP_TAC THEN FULL_SIMP_TAC std_ss [MU_SAT_AP,SUBF_REFL],

    (* <<>> *)
    REPEAT STRIP_TAC THEN
    Q.PAT_ASSUM `!t s1 s2. P SUBF P' ==>Q`
                (fn t => ASSUME_TAC (REWRITE_RULE [SUB_AP_DMD] t)) THEN
    FULL_SIMP_TAC std_ss [MU_SAT_DMD] THEN
    Q.PAT_ASSUM `BS t`
       (fn t => RES_TAC THEN NTAC 2 (POP_ASSUM (K ALL_TAC)) THEN
                POP_ASSUM
                    (fn t' => NTAC 2 (POP_ASSUM (K ALL_TAC)) THEN
                              POP_ASSUM
                                  (fn t'' =>
                                      NTAC 3 (POP_ASSUM (K ALL_TAC)) THEN
                                      ASSUME_TAC t'' THEN ASSUME_TAC t')) THEN
                    ASSUME_TAC t) THEN
    FULL_SIMP_TAC std_ss [BISIM_def] THEN EQ_TAC THEN REPEAT STRIP_TAC THEN
    RES_TAC THENL [
      Q.PAT_ASSUM `!a b. P ==> (Q=Q')`
                  (fn t => ASSUME_TAC (Q.SPECL [`s2'`,`q`] t)) THEN
      Q.EXISTS_TAC `s2'` THEN metisLib.METIS_TAC [],
      Q.PAT_ASSUM `!a b. P ==> (Q=Q')`
                  (fn t => ASSUME_TAC (Q.SPECL [`q`,`s1'`] t)) THEN
      Q.EXISTS_TAC `s1'` THEN metisLib.METIS_TAC []
    ],

    (* [[]] *)
    REPEAT STRIP_TAC THEN
    Q.PAT_ASSUM `!t s1 s2. P SUBF P' ==>Q` (fn t => ASSUME_TAC (REWRITE_RULE [SUB_AP_BOX] t))
    ...
    THENL [
      Induct_on `n` THENL [
        ...
        REPEAT STRIP_TAC THEN SIMP_TAC std_ss [STATES_def,ENV_UPDATE] THEN
        Q.PAT_ASSUM `!t s1 s2. P SUBF P' ==>Q`
                    (fn t => ASSUME_TAC (REWRITE_RULE [SUB_AP_MU] t)) THEN
        Q.PAT_ASSUM `!M1 M2 e1 e2. P`
          (fn t => ASSUME_TAC
                     (Q.SPECL
                          [`M1`,`M2`,
        ...
        POP_ASSUM (fn t => NTAC 2 (POP_ASSUM (K ALL_TAC)) THEN
                           ASSUME_TAC t) THEN
        POP_ASSUM_LIST (fn l => MAP_EVERY ASSUME_TAC (List.take(l,4))) THEN
        Q.SUBGOAL_THEN
           `!Q s1 s2.
               BS (s1,s2) ==>
...
THEN EQ_TAC THEN REPEAT STRIP_TAC THENL [
  Q.PAT_ASSUM `!s2. ?s1. BS(s1,s2)`
              (fn t => ASSUME_TAC t THEN ASSUME_TAC(Q.SPEC `s` t)) THEN
  FULL_SIMP_TAC std_ss [] THEN
  Q.PAT_ASSUM `!s1 s2. BS (s1,s2) ==> (s1 IN M1.S0 = s2 IN M2.S0)`
              (fn t => IMP_RES_TAC (Q.SPECL [`s1`,`s`] t)) THEN
  Q.PAT_ASSUM `!s. s IN M1.S0 ==> MU_SAT f M1 e1 s`
              (fn t => ASSUME_TAC (Q.SPEC `s1` t)) THEN
  IMP_RES_TAC MU_BISIM_STATES THEN NTAC 5 (POP_ASSUM (K ALL_TAC)) THEN
  POP_ASSUM (fn t => NTAC 3 (POP_ASSUM (K ALL_TAC)) THEN ASSUME_TAC t) THEN
  FULL_SIMP_TAC std_ss [],

  Q.PAT_ASSUM `!s1. ?s2. BS(s1,s2)`
       (fn t => Q.PAT_ASSUM `!s1. ?s2. BS(s1,s2)`
                            (fn t => ASSUME_TAC t THEN
                                     ASSUME_TAC(Q.SPEC `s` t))) THEN
  FULL_SIMP_TAC std_ss [] THEN
  Q.PAT_ASSUM `!s1 s2. BS (s1,s2) ==> (s1 IN M1.S0 = s2 IN M2.S0)`
              (fn t => IMP_RES_TAC (Q.SPECL [`s`,`s2`] t)) THEN
  Q.PAT_ASSUM `!s. s IN M2.S0 ==> MU_SAT f M2 e2 s`
              (fn t => ASSUME_TAC (Q.SPEC `s2` t)) THEN
  IMP_RES_TAC MU_BISIM_STATES THEN
