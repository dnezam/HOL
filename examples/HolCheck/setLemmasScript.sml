...
Theorem SET_SPEC = prove(``!P x. x IN { y | P y} = P x``, RW_TAC std_ss [SET_SPEC_CONV `` x IN { y | P y}``])

Theorem DIFF_OVER_INTER = prove(``!s t u. s SUBSET u /\ t SUBSET u ==> (u DIFF (s INTER t) = (u DIFF s) UNION (u DIFF t))``, RW_TAC std_ss [SUBSET_DEF,IN_DIFF,IN_INTER,IN_UNION,DIFF_DEF,EXTENSION,SET_SPEC] THEN PROVE_TAC [])


Theorem DIFF_OVER_UNION = prove(``!s t u. s SUBSET u /\ t SUBSET u ==> (u DIFF (s UNION t) = (u DIFF s) INTER (u DIFF t))``, RW_TAC std_ss [SUBSET_DEF,IN_DIFF,IN_INTER,IN_UNION,DIFF_DEF,EXTENSION,SET_SPEC] THEN PROVE_TAC [])

Theorem DIFF_OVER_BIGUNION = prove (``!P. (UNIV DIFF (BIGUNION {s | ?(n:num). s = (P n)})) = (BIGINTER {s | ?(n:num). (s = (UNIV DIFF (P n)))})``,RW_TAC std_ss [prove(``!P. BIGINTER {s | ?(n:num). s = UNIV DIFF P n} = {x | !(n:num). x IN UNIV DIFF P n}``,
...
Theorem DIFF_OVER_BIGINTER = prove(``!P. (UNIV DIFF (BIGINTER {s | ?n. s = (P n)})) = (BIGUNION {s | ?n. (s = (UNIV DIFF (P n)))})``,
...
Theorem SET_GSPEC = prove(``!P x. { x | P x} = \x. P x``,
RW_TAC std_ss [EXTENSION,SET_SPEC,SPECIFICATION])

Theorem UNIV_DIFF_EQ = prove (``!s t. (UNIV DIFF s = UNIV DIFF t) = (s = t)``,RW_TAC std_ss [IN_UNIV,DIFF_DEF,EXTENSION,SET_SPEC])

Theorem BIGINTER_LEMMA1 =
prove (``!P Q. (!n. P n = Q n) ==> (BIGINTER {s | ?n. s = P n} = BIGINTER {s | ?n. s = Q n})``,
RW_TAC std_ss [BIGINTER,SET_SPEC,EXTENSION])

Theorem BIGUNION_LEMMA1 = prove(``!P Q. (!n. P n = Q n) ==> (BIGUNION {s | ?n. s = P n} = BIGUNION {s | ?n. s = Q n})``,
RW_TAC std_ss [BIGUNION,SET_SPEC,EXTENSION])

Theorem UNIV_DIFF_SUBSET = prove (``!s t. (UNIV DIFF s SUBSET UNIV DIFF t) = (t SUBSET s)``,
FULL_SIMP_TAC std_ss [DIFF_DEF,SUBSET_DEF,SET_SPEC,IN_UNIV]
THEN PROVE_TAC [])

Theorem SUBSET_DEST = prove(``!s t. s SUBSET t = s SUBSET t \/ (s=t)``,PROVE_TAC [SUBSET_DEF])

Theorem EQ_IMP_SUBSET = prove(``!P Q. (P = Q) ==> (P SUBSET Q)``,PROVE_TAC [SUBSET_DEF])

Theorem bus1 = prove(``!(Q :num -> 'state -> bool) (n' :num).
...
Theorem bus2 = prove(``!P Q. (P = Q) ==> (BIGUNION P = BIGUNION Q)``,PROVE_TAC [])

Theorem BIGUNION_PSPLIT = prove(``!Q n'. BIGUNION {P | ?n.  ~(n=0) /\  (P = Q n)} =
...
Theorem BIGUNION_PSPLIT2 = prove(``!Q n'. BIGUNION {P | ?n.  (P = Q n)} =
...
Theorem BIGUNION_SPLIT = prove(``!Q n'. BIGUNION {P | ?n. (P = Q n)} =
      BIGUNION {P | ?n. n <= n' /\ (P = Q n)} UNION BIGUNION {P | ?n. n >= n' /\ (P = Q n)}``,
...
Theorem BIGUNION_SPLIT_EXISTS = prove(``?n'. !Q. BIGUNION {P | ?n. (P = Q n)} =
      BIGUNION {P | ?n. n <= n' /\ (P = Q n)} UNION BIGUNION {P | ?n. n >= n' /\ (P = Q n)}``,METIS_TAC [BIGUNION_SPLIT])

Theorem SUBSET_DEST2 = prove(``!s t. s SUBSET t = s PSUBSET t \/ (s=t)``,SIMP_TAC std_ss [SUBSET_DEF,PSUBSET_DEF,IN_DEF] THEN PROVE_TAC [])

Theorem PSUBSET_CARD_LT = prove(``!s t. FINITE s /\ FINITE t ==> (s PSUBSET t ==> CARD s < CARD t)``,PROVE_TAC [CARD_PSUBSET])

Theorem SUBSET_CARD_LTE = prove(``!s t. FINITE s /\ FINITE t ==> (s SUBSET t ==> CARD s <= CARD t)``,PROVE_TAC [CARD_SUBSET])

Theorem GEN_PCHAIN_CARD_NO_UB = prove(``!(P:num -> 'state -> bool). (!n. CARD (P n) < (CARD (P (SUC n)))) ==> !m. ?n. (CARD (P n) > m)``,
...
Theorem BIGUNION_LEM2 = prove(``!P Q. (!n. P n SUBSET Q n) ==> BIGUNION {p | ?n. (p = P n)} SUBSET BIGUNION {p | ?n. (p = Q n)}``,
SIMP_TAC std_ss [BIGUNION,SET_SPEC,SUBSET_DEF] THEN PROVE_TAC [])

Theorem SUBSET_EQ = prove(``!s t. s SUBSET t /\ t SUBSET s = (s=t)``,
REPEAT STRIP_TAC THEN EQ_TAC THENL [
SIMP_TAC std_ss [SUBSET_ANTISYM],
SIMP_TAC std_ss [SUBSET_DEF,IN_DEF]])

Theorem BIGUNION_SUBSET_IMP = prove(``!P Q. (!n. P n SUBSET Q n) ==> BIGUNION {P' | ?n. P' = P n} SUBSET BIGUNION {Q' | ?n. Q' = Q n}``,
...
Theorem LEFT_BIGUNION_SUBSET = prove(``!X P. (!n. P n SUBSET X) ==> BIGUNION {Q | ?n. Q = P n} SUBSET X``,
REPEAT STRIP_TAC
THEN FULL_SIMP_TAC std_ss [BIGUNION,SUBSET_DEF,SET_SPEC]
THEN REPEAT STRIP_TAC
THEN ASSUM_LIST PROVE_TAC)

Theorem BIGINTER_SUBSET_IMP = prove(``!P Q. (!n. P n SUBSET Q n) ==> BIGINTER {P' | ?n. P' = P n} SUBSET BIGINTER {Q' | ?n. Q' = Q n}``,
...
Theorem LEFT_BIGINTER_SUBSET = prove(``!X P. (!n. X SUBSET P n) ==> X SUBSET BIGINTER {Q | ?n. Q = P n}``,
REPEAT STRIP_TAC
THEN FULL_SIMP_TAC std_ss [BIGINTER,SUBSET_DEF,SET_SPEC]
THEN REPEAT STRIP_TAC
THEN ASSUM_LIST PROVE_TAC)

Theorem BIGINTER_SPLIT = prove(``!Q n'. BIGINTER {P | ?n. P = Q n} =
...
Theorem BIGINTER_INTER = prove(``!s1 s2. BIGINTER (s1 UNION s2) = (BIGINTER s1) INTER (BIGINTER s2)``,
...
Theorem BIGINTER_SUBSET = prove(``!P Q. (!n. P n SUBSET Q n) ==> BIGINTER {P' | ?n. P' = P n} SUBSET BIGINTER {Q' | ?n. Q' = Q n}``,
REPEAT STRIP_TAC THEN SIMP_TAC std_ss [BIGINTER] THEN SIMP_TAC std_ss [SUBSET_DEF,SET_SPEC] THEN FULL_SIMP_TAC std_ss [SUBSET_DEF] THEN ASSUM_LIST PROVE_TAC)

Theorem BOOL_UNIV_FINITE = prove(``FINITE (UNIV:bool->bool)``,
...
Theorem UNIV_CROSS_UNIV = prove(``(UNIV:bool -> bool) CROSS (UNIV:'state->bool) = (UNIV:(bool # 'state)->bool)``,
SIMP_TAC std_ss [UNIV_DEF,CROSS_DEF,FST,SND,IN_DEF,SET_GSPEC])

Theorem IN_APPLY = prove(``!P x. x IN P = P x``,SIMP_TAC std_ss [IN_DEF] THEN BETA_TAC)
...
Theorem INF_STRING_UNIV = prove(``INFINITE (UNIV:string->bool)``,
REWRITE_TAC [INFINITE_UNIV]
THEN Q.EXISTS_TAC `PRIME`
THEN METIS_TAC [PRIME_11,PRIME_NOT_ONTO])

Theorem NOT_IN_FIN_STRING_SET = prove(``!s. FINITE s ==> ?(x:string). ~(x IN s)``,
 REPEAT STRIP_TAC
...
Theorem PUSH_IMP_THM = prove(``!a b c. a ==> b ==> c = b ==> a ==> c``,PROVE_TAC [])
