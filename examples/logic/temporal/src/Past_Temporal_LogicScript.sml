...
Ancestors
  num prim_rec arithmetic pair Temporal_Logic
Libs
  Rsyntax numLib schneiderUtils tautLib
...
val PEVENTUAL = new_definition("PEVENTUAL",
  “PEVENTUAL a t0 = ?t:num. t<=t0 /\ a t”);
...
val PWHEN_INIT = TAC_PROOF(
        ([],“ (a PWHEN b) 0 = a 0 \/ ~b 0 ”),
        ONCE_REWRITE_TAC[DECIDE “a\/~b = (a/\b)\/~b”]
...
val PSUNTIL_REC = TAC_PROOF(
        ...
        THEN ONCE_REWRITE_TAC[DECIDE “a\/b = a\/(~a/\b)”]
        THEN EQ_TAC THEN REPEAT STRIP_TAC
        THENL[
...
val PSWHEN_REC = TAC_PROOF(
        ...
        THENL[
            ...
            THENL[POP_NO_ASSUM 1 MATCH_MP_TAC THEN
...
val PSBEFORE_REC = TAC_PROOF(
        ...
        THENL[
            ...
            EXISTS_TAC“SUC t” THEN ASM_REWRITE_TAC[LESS_EQ_REFL]
            THEN REWRITE_TAC[
                   EQT_ELIM(ARITH_CONV“(SUC t<=x /\ x<=SUC t) = (x = SUC t)”)]
            THEN GEN_TAC THEN DISCH_TAC THEN ASM_REWRITE_TAC[],
            EXISTS_TAC “delta:num”
            THEN IMP_RES_TAC(EQT_ELIM(ARITH_CONV“delta<=t ==> delta<=SUC t”))
            THEN ASM_REWRITE_TAC[]
...
val PUNTIL_REC = TAC_PROOF(
 ...
 THENL[
     ...
     THENL[
         ...
         EXISTS_TAC “delta:num”
         THEN IMP_RES_TAC
                (EQT_ELIM(ARITH_CONV“delta<=t ==> delta<=SUC t”))
         THEN ASM_REWRITE_TAC[]
...
val PSBEFORE_FIX = TAC_PROOF(
        ...
        FIXPOINT_CHARACTERIZATION_TAC PSBEFORE_INIT PSBEFORE_REC THEN TAUT_TAC);


val PUNTIL_FIX = TAC_PROOF(
        ([],“ (y = \t. b t \/ a t /\ PNEXT y t) = (y = (a PUNTIL b))”),
        FIXPOINT_CHARACTERIZATION_TAC PUNTIL_INIT PUNTIL_REC THEN TAUT_TAC);


val PWHEN_FIX = TAC_PROOF(
        ([],“ (y = \t. a t /\ b t \/ ~b t /\ PNEXT y t) = (y = (a PWHEN b))”),
        FIXPOINT_CHARACTERIZATION_TAC PWHEN_INIT PWHEN_REC THEN TAUT_TAC);


val PBEFORE_FIX = TAC_PROOF(
        ([],“ (y = \t. ~b t /\ (a t \/ PNEXT y t)) = (y = (a PBEFORE b))”),
        FIXPOINT_CHARACTERIZATION_TAC PBEFORE_INIT PBEFORE_REC THEN TAUT_TAC);
...
val SBEFORE_AS_SUNTIL1 = TAC_PROOF(
        ...
        CONV_TAC(X_FUN_EQ_CONV “t0:num”)
        THEN REWRITE_TAC[SBEFORE_SIGNAL,SUNTIL_SIGNAL] THEN BETA_TAC
        THEN REWRITE_TAC[DECIDE “(~b/\~(a/\~b)) = (~a/\~b)”]
        THEN GEN_TAC THEN EQ_TAC THEN REPEAT STRIP_TAC
        THENL[
            ALL_TAC,
            EXISTS_TAC“delta:num” THEN ASM_REWRITE_TAC[]
...
val future_cnf_thm = TAC_PROOF(
    ...
    THEN REWRITE_TAC[WHEN_SIGNAL,SWHEN_SIGNAL,UNTIL_SIGNAL,SUNTIL_SIGNAL,
                     SBEFORE_SIGNAL]
    THEN BETA_TAC
    ...
    THENL[
        ...
        THEN RES_TAC
          (* cases (delta<delta') and (delta'<delta) are seen to be wrong *)
...
val before_cnf_thm = TAC_PROOF(
  ...
  THEN ONCE_REWRITE_TAC[DECIDE “(~a= ~b/\~c) = (a = b\/c)”]
...
val NEXT_OR_PSUNTIL_SEPARATE = TAC_PROOF(
        ...
        CONV_TAC(X_FUN_EQ_CONV “t:num”) THEN GEN_TAC THEN BETA_TAC
        THEN CONV_TAC(LHS_CONV(REWRITE_CONV[PSUNTIL_WRAP]))
        THEN REWRITE_TAC[DECIDE “a\/c\/b = (a\/c) \/ b”]
        THEN SUBST1_TAC(BETA_RULE
...
val NEXT_OR_PBEFORE_SEPARATE = TAC_PROOF(
        ...
        CONV_TAC(X_FUN_EQ_CONV “t:num”) THEN GEN_TAC THEN BETA_TAC
        THEN CONV_TAC(LHS_CONV(REWRITE_CONV[PBEFORE_WRAP]))
        THEN REWRITE_TAC[DECIDE “a\/~c/\(b\/d) = (a\/~c) /\ ((a\/b)\/d)”]
        THEN SUBST1_TAC(BETA_RULE
...
val SUNTIL_AND_PNEXT_SEPARATE = TAC_PROOF(
  ...
  THEN MATCH_MP_TAC(DECIDE “(b ==> (a=c)) ==> ((b ==> a) = (b ==> c))”)
  ...
  THEN REWRITE_TAC[DECIDE “a/\~(a/\c/\b) = a/\(c==>~b)”]
  ...
  THENL[
      ...
      THENL[
          ...
          THEN ASM_REWRITE_TAC[DECIDE “(c==>~b)= ~(b/\c)”]
      ...
      THENL[ASM_REWRITE_TAC[ADD_CLAUSES,EQT_ELIM(ARITH_CONV“~(x<0)”)],ALL_TAC]
      THEN GEN_TAC THEN DISCH_TAC THEN RES_TAC
      THEN ASM_REWRITE_TAC[DECIDE “~(b/\c)=(c==>~b)”]
...
val SUNTIL_OR_PNEXT_SEPARATE = TAC_PROOF(
  ...
  THEN BETA_TAC
  THEN REWRITE_TAC[DECIDE “~a==>b = (a\/b)”]
  THEN MATCH_MP_TAC(DECIDE“(a=b) ==> ((c\/a) = (c\/b))”)
  THEN MATCH_MP_TAC(DECIDE“(a=b) ==> ((c/\a) = (c/\b))”)
...
val SUNTIL_AND_PSUNTIL_SEPARATE = TAC_PROOF(
  ...
  THENL[
      (* -------------------------------------------------------------------
          Proving that the lhs implies the rhs. The point of time where the
          event b /\ (c PSUNTIL d) holds first after t, is t1, and the first
          point of time before t1 where d held is delta. Therefore, we
          consider the cases:
         ------------------------------------------------------------------- *)
      DISJ_CASES_TAC(EQT_ELIM(ARITH_CONV“(delta<=t)\/(t<delta)”))
      THENL[
        (* ---------------------------------------------------------------
            delta<=t holds, so we take the first disjunct, i.e. we must
            prove
        ...
        THENL[
          ...
          THEN IMP_RES_TAC LESS_EQ_TRANS
          THEN ASM_REWRITE_TAC[
                 EQT_ELIM(ARITH_CONV“(delta<SUC t2) = (delta<=t2)”)]
          THEN STRIP_TAC
          ],
        (* ---------------------------------------------------------------
...
val SUNTIL_AND_PBEFORE_SEPARATE = TAC_PROOF(
  ...
  THENL[
    (* ---------------------------------------------------------------------
        First, we have to show that the lhs implies the rhs, when b
        becomes true at time t1>=t, and d is false for all t'<= t1. We
        choose the first disjunct, i.e.,
           (c PBEFORE d) /\ ((a /\ ~NEXT c /\ ~NEXT d) SUNTIL b) to prove
        this.
       --------------------------------------------------------------------- *)
    DISJ1_TAC THEN REPEAT STRIP_TAC
    THENL[
    ...
    THENL[
        (* -----------------------------------------------------------------
            delta<=t holds, so we take the first disjunct, i.e. we must
            prove (c PBEFORE d) /\ ((a /\ ~NEXT d) SUNTIL b)
           ----------------------------------------------------------------- *)
        DISJ1_TAC THEN REPEAT STRIP_TAC
        THENL[
...
val BEFORE_AND_PSUNTIL_SEPARATE = TAC_PROOF(
  ...
  THEN REWRITE_TAC[DECIDE “(a/\b)/\~d = (a/\~d)/\b”]
  ...
  THEN MATCH_MP_TAC
         (TAC_PROOF(([],“!a b1 b2 c. (b1=b2)==> ((a\/b1)\/c=a\/b2\/c)”),
                    REPEAT GEN_TAC THEN TAUT_TAC))
  THEN MY_MP_TAC
          “!a1 a2 b1 b2.
...
val BEFORE_AND_PBEFORE_SEPARATE = TAC_PROOF(
  ...
  THEN MATCH_MP_TAC
        (TAC_PROOF(([],“!a b1 b2 c. (b1=b2)==> ((a\/b1)\/c=a\/b2\/c)”),
                   REPEAT GEN_TAC THEN TAUT_TAC))
  THEN MY_MP_TAC
    “!a1 a2 b1 b2. (a1=a2) /\(b1=b2) ==> ((a1 SUNTIL b1) t = (a2 SUNTIL b2) t)”
  THENL[
      REPEAT STRIP_TAC THEN ASM_REWRITE_TAC[],
      DISCH_TAC]
  THEN POP_ASSUM MATCH_MP_TAC THEN REWRITE_TAC[]
   ...
   “((\t. a t \/ (b PBEFORE c) t) SUNTIL d) =
    \t. ((b PBEFORE c) t \/
         ((\t. d t \/ NEXT b t) BEFORE (\t. ~a t /\ ~d t)) t)
               /\
        ((\t. ~c t \/ ((\t. d t \/ NEXT b t) BEFORE (\t. ~a t /\ ~d t)) t)
           SUNTIL d)
        t
  ”),
  CONV_TAC(X_FUN_EQ_CONV “t:num”) THEN GEN_TAC
...
val BEFORE_LEFT_PNEXT_SEPARATE = TAC_PROOF(
  ...
  THEN BETA_TAC
  THEN REWRITE_TAC[DECIDE “a/\(b\/c) = (a/\b)\/(a/\c)”]
  THEN MATCH_MP_TAC(DECIDE “ (a=b) ==> ((c\/a) = (c\/b)) ”)
  THEN MATCH_MP_TAC(DECIDE “ (a=b) ==> ((c/\a) = (c/\b)) ”)
...
val SEPARATE_PNEXT_THM = TAC_PROOF(
  ...
  THENL[
      ...
      THEN REWRITE_TAC[NEXT] THEN BETA_TAC THEN TAUT_TAC,
...
val PSUNTIL_AND_SUNTIL_SEPARATE = TAC_PROOF(
  ...
  THENL[
      REWRITE_TAC[INITIALISATION] THEN BETA_TAC
      THEN REWRITE_TAC[INITIALISATION,SUNTIL_REC] THEN TAUT_TAC,
      ALL_TAC]
  THEN ONCE_REWRITE_TAC[PSUNTIL_REC] THEN BETA_TAC
...
val PSUNTIL_AND_BEFORE_SEPARATE = prove(
  ...
  THENL[
      REWRITE_TAC[INITIALISATION] THEN BETA_TAC
      THEN REWRITE_TAC[INITIALISATION,BEFORE_REC] THEN TAUT_TAC,
      ALL_TAC]
  THEN ONCE_REWRITE_TAC[PSUNTIL_REC] THEN BETA_TAC
  THEN REWRITE_TAC[PNEXT,PSNEXT,PRE,ZERO_LEMMA] THEN BETA_TAC
  THEN POP_ASSUM SUBST1_TAC
...
val PALWAYS_NEXT = TAC_PROOF(
  ...
  THEN DISCH_TAC
  THEN REWRITE_TAC[EQT_ELIM(ARITH_CONV“(n=0) = ~(0<n)”)]
  THEN REWRITE_TAC[DECIDE “b\/~a = a==>b”]
  THEN DISCH_TAC THEN IMP_RES_TAC LESS_ADD_1
  THEN RULE_ASSUM_TAC(REWRITE_RULE[ADD_CLAUSES,num_CONV“1”])
...
val PRENEX_NEXT_NORMAL_FORM = TAC_PROOF(
  ...
  THEN REWRITE_TAC[
    DECIDE “a t \/ InitPoint t = InitPoint t \/ a t”]
  THEN REWRITE_TAC[
    DECIDE “a t /\ ~InitPoint t = ~InitPoint t /\ a t”]
...
val PNEXT_INWARDS_NORMAL_FORM = TAC_PROOF(
  ...
  ”),
    let val PSNEXT_AS_PNEXT = TAC_PROOF(
            ...
            THEN REWRITE_TAC[DECIDE “~a/\(a\/b) = ~a/\b”]
...
Theorem INITIALISATION = INITIALISATION

Theorem RECURSION = RECURSION

Theorem FIXPOINTS = FIXPOINTS

Theorem SUNTIL_EXPRESSIVE = SUNTIL_EXPRESSIVE
Theorem UNTIL_EXPRESSIVE = UNTIL_EXPRESSIVE
Theorem WHEN_EXPRESSIVE = WHEN_EXPRESSIVE
Theorem SWHEN_EXPRESSIVE = SWHEN_EXPRESSIVE
Theorem BEFORE_EXPRESSIVE = BEFORE_EXPRESSIVE
Theorem SBEFORE_EXPRESSIVE = SBEFORE_EXPRESSIVE
Theorem PSUNTIL_EXPRESSIVE = PSUNTIL_EXPRESSIVE
Theorem PUNTIL_EXPRESSIVE = PUNTIL_EXPRESSIVE
Theorem PWHEN_EXPRESSIVE = PWHEN_EXPRESSIVE
Theorem PSWHEN_EXPRESSIVE = PSWHEN_EXPRESSIVE
Theorem PBEFORE_EXPRESSIVE = PBEFORE_EXPRESSIVE
Theorem PSBEFORE_EXPRESSIVE = PSBEFORE_EXPRESSIVE

Theorem NEGATION_NORMAL_FORM = NEGATION_NORMAL_FORM
Theorem CONJUNCTIVE_NORMAL_FORM = CONJUNCTIVE_NORMAL_FORM
Theorem DISJUNCTIVE_NORMAL_FORM = DISJUNCTIVE_NORMAL_FORM
Theorem PRENEX_NEXT_NORMAL_FORM = PRENEX_NEXT_NORMAL_FORM
Theorem NEXT_INWARDS_NORMAL_FORM = NEXT_INWARDS_NORMAL_FORM
Theorem PNEXT_INWARDS_NORMAL_FORM = PNEXT_INWARDS_NORMAL_FORM

Theorem SIMPLIFY = SIMPLIFY
Theorem MORE_EVENT = MORE_EVENT
Theorem IMMEDIATE_EVENT = IMMEDIATE_EVENT
Theorem NO_FUTURE_EVENT = NO_FUTURE_EVENT
Theorem NO_PAST_EVENT = NO_PAST_EVENT
Theorem SOME_FUTURE_EVENT = SOME_FUTURE_EVENT
Theorem SOME_PAST_EVENT = SOME_PAST_EVENT

Theorem SEPARATE_NEXT_THM = SEPARATE_NEXT_THM
Theorem SEPARATE_SUNTIL_THM = SEPARATE_SUNTIL_THM
Theorem SEPARATE_BEFORE_THM = SEPARATE_BEFORE_THM

Theorem SEPARATE_PNEXT_THM = SEPARATE_PNEXT_THM
Theorem SEPARATE_PSUNTIL_THM = SEPARATE_PSUNTIL_THM
