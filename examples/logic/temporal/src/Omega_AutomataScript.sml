...
Theorem DET_OMEGA_EXISTS_FORALL_THEOREM[local]:
  (?q:num->'state.
     (q t0 = InitVal) /\
...
Proof
  ...
  THENL[
      ...
      (* ------------------------------------------------------------------ *)
      STRIP_ASSUME_TAC(EXISTENCE(REWRITE_RULE[ADD1](BETA_RULE
          (ISPECL[“InitVal:'state”,
                  “\x:'state.\t:num.TransRel((i(t+t0)):'input,x):'state”]
                  num_Axiom_old))))
      THEN EXISTS_TAC “\t:num.fn1(t-t0):'state”
      THEN FIRST_X_ASSUM
             (Q.SPEC_THEN ‘\t:num.fn1(t-t0):'state’ STRIP_ASSUME_TAC)
  ...
  “!a b.
     ...
     =
         (?q. ~q t0 /\
              ...
              ?t1.!t2. ~q(t1+(t2+t0)) \/ b(t1+(t2+t0)))”,
  REPEAT GEN_TAC
  THEN PURE_ONCE_REWRITE_TAC[DECIDE “(a=b)=(~a= ~b)”]
...
val FG_AS_NDET_GF = prove(
  ...
  THENL[
      ...
      THENL[
          ...
          THEN UNDISCH_HD_TAC
          THEN REWRITE_TAC[ADD_CLAUSES,ADD_ASSOC] THEN TAUT_TAC
          ],
      DISCH_TAC]
...
val WHEN2BUECHI_THM = prove(
  ...
  THENL[
      ...
      THEN DISCH_TAC
      THEN ONCE_REWRITE_TAC[DECIDE “(a:bool=b) = (b=a)”]
      THEN MY_MP_TAC “
                  (!n. EVENTUAL b n)
                  ==> !n. (a WHEN b) n = (a SWHEN b) n”
      THENL[
...
val UNTIL2BUECHI_THM = TAC_PROOF(
  ...
  THEN REWRITE_TAC[DECIDE“(if (a ==> b) then b else l) = (~b ==> a /\ l)”]
  THEN REWRITE_TAC[DECIDE“(l \/(a==>b)) = (~l ==> (~a\/b))”]);
...
val BEFORE2BUECHI_THM = TAC_PROOF(
  ...
  THEN REWRITE_TAC[DECIDE“(l = ~b/\(a\/l1)) = (~l = ~b ==>~a/\~l1)”]
  THEN ONCE_REWRITE_TAC[DECIDE“(~l ==> a \/b) = (~l ==> ~~a \/ b)”]
...
val ALWAYS2BUECHI_THM = TAC_PROOF(
        ...
        THEN REWRITE_TAC[DECIDE “(~a ==> ~l) =( l ==> a)”]);
...
val EVENTUAL2BUECHI_THM = TAC_PROOF(
        ...
        THEN REWRITE_TAC[DECIDE “(~a ==> l) = (a \/ l)”]);
...
val PSNEXT_PSBEFORE2OMEGA = TAC_PROOF(
        ...
        THENL[
            ...
            THEN ASM_REWRITE_TAC[INITIALISATION,LESS_REFL]
            THEN IMP_RES_TAC LESS_ADD_1
            THEN POP_ASSUM SUBST1_TAC THENL[TAUT_TAC,ALL_TAC]
            THEN REWRITE_TAC[num_CONV“1”,ADD_CLAUSES,PRE]
            THEN CONV_TAC(RHS_CONV(ONCE_REWRITE_CONV[RECURSION]))
            THEN BETA_TAC THEN REWRITE_TAC[PSNEXT,ZERO_LEMMA,PRE]
            ]);
...
val AUTOMATON_PSBEFORE_CLOSURE = TAC_PROOF(
        ...
        THEN REWRITE_TAC[AUTOMATON_PSUNTIL_CLOSURE]
        THEN BETA_TAC
        THEN REWRITE_TAC[DECIDE “a/\~b\/~b/\q = ~b/\(a\/q)”]);
...
val AUTOMATON_PWHEN_CLOSURE = TAC_PROOF(
  ...
  THEN REWRITE_TAC[AUTOMATON_PBEFORE_CLOSURE]
  THEN BETA_TAC
  THEN REWRITE_TAC[DECIDE“~(~a/\b)/\(a/\b\/q) = a/\b\/~b/\q”]);
...
val BUECHI_TRANSLATION = TAC_PROOF(
  ...
  REWRITE_TAC[AUTOMATON_NEXT_CLOSURE]
  THEN REWRITE_TAC[DECIDE “a/\(b/\c) = (a/\b)/\c”]
  THEN REWRITE_TAC[DECIDE “b\/a/\q = ~b==>a/\q”]
  THEN REWRITE_TAC
...
val CO_BUECHI_BEFORE_CLOSURE = prove(
  ...
  THEN REWRITE_TAC[DECIDE “a/\b/\c ==> d = a ==> b ==> c ==>d”]
  ...
  THENL[
      ...
      THENL[
          ...
          (* -----------------------------------------------------
            ...
            ----------------------------------------------------- *)
      THEN ONCE_REWRITE_TAC[DECIDE “a = ~~a”]
...
val BEFORE_AS_CO_BUECHI = TAC_PROOF(
  ...
  THENL[
      ...
      ONCE_REWRITE_TAC[DECIDE “a \/ b = (~b ==> a)”]
      THEN CONV_TAC(DEPTH_CONV NOT_FORALL_CONV)
      THEN REWRITE_TAC[] THEN DISCH_TAC
      THEN EXISTS_TAC “q:num->bool” THEN ASM_REWRITE_TAC[]
...
Theorem REPETITION_LEMMA[local]:
...
Theorem REPETITION_BOOL_LEMMA[local]:
  (!s:num->'state.?x0 l. (0<l) /\ (s(x0)=s(x0+l))) ==>
  !(s:num->'state) (q:num->bool).
    ?x0 l. 0<l /\ (s x0 =s(x0+l)) /\ (q x0 = q(x0+l))
Proof
  REPEAT STRIP_TAC
...
val ELGOT2_THM = TAC_PROOF(
  ...
  GEN_TAC THEN ONCE_REWRITE_TAC[DECIDE “ (a=b) = (~a= ~b)”]
  THEN CONV_TAC(REPEATC(CHANGED_CONV(DEPTH_CONV
                  (NOT_FORALL_CONV ORELSEC NOT_EXISTS_CONV))))
  THEN SUBST1_TAC(BETA_RULE(SPEC“\(p:'a->bool) (x:'a).~PHI p x”ELGOT1_THM))
  THEN REWRITE_TAC[DECIDE “((qz ==> ~PHI) /\ qx) = ~(qx ==> PHI /\ qz)”]);





Theorem DET_OMEGA_EXISTS_FORALL_THEOREM =
                 DET_OMEGA_EXISTS_FORALL_THEOREM
Theorem NEG_DET_AUTOMATA = NEG_DET_AUTOMATA
Theorem OMEGA_CONJ_CLOSURE = OMEGA_CONJ_CLOSURE
Theorem OMEGA_DISJ_CLOSURE = OMEGA_DISJ_CLOSURE

Theorem BOOLEAN_CLOSURE_G = BOOLEAN_CLOSURE_G
Theorem BOOLEAN_CLOSURE_F = BOOLEAN_CLOSURE_F
Theorem BOOLEAN_CLOSURE_FG = BOOLEAN_CLOSURE_FG
Theorem BOOLEAN_CLOSURE_GF = BOOLEAN_CLOSURE_GF

Theorem BOREL_HIERARCHY_G = BOREL_HIERARCHY_G
Theorem BOREL_HIERARCHY_F = BOREL_HIERARCHY_F
Theorem BOREL_HIERARCHY_FG = BOREL_HIERARCHY_FG

(* ----------------- from file templog2buechi.sml ------------------ *)
Theorem TEMP_OPS_DEFS_TO_OMEGA = TEMP_OPS_DEFS_TO_OMEGA

(* ----------------- from file automaton_closures.sml ------------------ *)
Theorem AUTOMATON_TEMP_CLOSURE = AUTOMATON_TEMP_CLOSURE
Theorem BUECHI_TRANSLATION = BUECHI_TRANSLATION

(* ----------------- from file co_buechi_closures.sml ------------------ *)
Theorem CO_BUECHI_CONJ_CLOSURE = CO_BUECHI_CONJ_CLOSURE
Theorem CO_BUECHI_DISJ_CLOSURE = CO_BUECHI_DISJ_CLOSURE
Theorem CO_BUECHI_NEXT_CLOSURE = CO_BUECHI_NEXT_CLOSURE
Theorem CO_BUECHI_SUNTIL_CLOSURE = CO_BUECHI_SUNTIL_CLOSURE
Theorem CO_BUECHI_UNTIL_CLOSURE = CO_BUECHI_UNTIL_CLOSURE
Theorem CO_BUECHI_SBEFORE_CLOSURE = CO_BUECHI_SBEFORE_CLOSURE
Theorem CO_BUECHI_BEFORE_CLOSURE = CO_BUECHI_BEFORE_CLOSURE
Theorem CO_BUECHI_SWHEN_CLOSURE = CO_BUECHI_SWHEN_CLOSURE
Theorem CO_BUECHI_WHEN_CLOSURE = CO_BUECHI_WHEN_CLOSURE

(* ----------------- from file tempop_as_co_buechi.sml ------------------ *)
Theorem NEXT_AS_CO_BUECHI = NEXT_AS_CO_BUECHI
Theorem SUNTIL_AS_CO_BUECHI = SUNTIL_AS_CO_BUECHI
Theorem UNTIL_AS_CO_BUECHI = UNTIL_AS_CO_BUECHI
Theorem SBEFORE_AS_CO_BUECHI = SBEFORE_AS_CO_BUECHI
Theorem BEFORE_AS_CO_BUECHI = BEFORE_AS_CO_BUECHI
Theorem SWHEN_AS_CO_BUECHI = SWHEN_AS_CO_BUECHI
Theorem WHEN_AS_CO_BUECHI = WHEN_AS_CO_BUECHI

(* ----------------- from file periodic.sml ------------------ *)
Theorem BUECHI_PERIODIC_MODEL = BUECHI_PERIODIC_MODEL
Theorem BUECHI_PERIODIC_REDUCTION_THM = BUECHI_PERIODIC_REDUCTION_THM
Theorem BUECHI_PROP_REDUCTION = BUECHI_PROP_REDUCTION

(* ----------------- from file linord2buechi.sml ------------------ *)
Theorem EQUALITY_THM = EQUALITY_THM
Theorem LESS_THM = LESS_THM
Theorem FORALL_EXISTS_THM = FORALL_EXISTS_THM
Theorem EXISTS_FORALL_THM = EXISTS_FORALL_THM
Theorem ELGOT_LEMMA = ELGOT_LEMMA
Theorem ELGOT1_THM = ELGOT1_THM
Theorem ELGOT2_THM = ELGOT2_THM
